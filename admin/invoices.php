	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id']) && isset($_GET['csrf'])) {
		if (hash_equals($_SESSION['csrf_token'], $_GET['csrf'])) {
			...
			if ($stmt->affected_rows > 0) {
				...
				echo "<script>alert('Error: Unable to delete invoice.');</script>";
			}
...
<html>
<head>
<title>Elegance Salon || Customer List</title>
...
?>
						 ...
						 <tr> 
						 	...
						 	<td><?php  echo $row['PostingDate'];?></td> 
								...
								<a href="#" onclick="confirmDelete(<?php echo $row['BillingId']; ?>, '<?php echo $_SESSION['csrf_token']; ?>')">Delete</a>
	...
	<!-- Classie -->
		...
		<script>
            ...
            if (confirm("Are you sure you want to delete this invoice?")) {
                window.location.href = "invoices.php?action=delete&id=" + id + "&csrf=" + csrfToken;
            }
        }
