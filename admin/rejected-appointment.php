	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id']) && isset($_GET['csrf'])) {
		if (hash_equals($_SESSION['csrf_token'], $_GET['csrf'])) {
			...
			if ($stmt->affected_rows > 0) {
				echo "<script>alert('rejected Appointment deleted successfully.');</script>";
			} else {
				echo "<script>alert('Error: Unable to delete rejected appointment.');</script>";
			}
...
<html>
<head>
<title>Elegance Salon || Rejected Appointment</title>
	...
	<!-- Classie -->
		...
		<script>
            ...
            if (confirm("Are you sure you want to delete this rejected-appointment?")) {
                window.location.href = "rejected-appointment.php?action=delete&id=" + id + "&csrf=" + csrfToken;
            }
        }
