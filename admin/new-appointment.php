...
//   } else{
	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id']) && isset($_GET['csrf'])) {
		if (hash_equals($_SESSION['csrf_token'], $_GET['csrf'])) {
			$id = intval($_GET['id']);
			$stmt = $con->prepare("DELETE FROM tblappointment WHERE ID = ?");
			$stmt->bind_param("i", $id);
			$stmt->execute();
	
			if ($stmt->affected_rows > 0) {
				echo "<script>alert('new Appointment deleted successfully.');</script>";
			} else {
				echo "<script>alert('Error: Unable to delete new appointment.');</script>";
			}
...
<html>
<head>
<title>Elegance Salon || New Appointment</title>
							 ...
							 <td><?php  echo $row['AptNumber'];?></td>
							  ...
							  <td><?php  echo $row['AptDate'];?></td>
							   <td><?php  echo $row['AptTime'];?></td>
							    <td>
									<a href="view-appointment.php?viewid=<?php echo $row['ID'];?>">View</a> |
									<a href="#" onclick="confirmDelete(<?php echo $row['ID']; ?>, '<?php echo $_SESSION['csrf_token']; ?>')">Delete</a>
							</td> 
						</tr>   <?php 
$cnt=$cnt+1;
}?></tbody> </table> 
	...
	<!-- Classie -->
		...
		<script>
            ...
            if (confirm("Are you sure you want to delete this new-appointment?")) {
                window.location.href = "new-appointment.php?action=delete&id=" + id + "&csrf=" + csrfToken;
            }
        }
