	...
	if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id']) && isset($_GET['csrf'])) {
		if (hash_equals($_SESSION['csrf_token'], $_GET['csrf'])) {
			...
			if ($stmt->affected_rows > 0) {
				echo "<script>alert('accepted Appointment deleted successfully.');</script>";
			} else {
				echo "<script>alert('Error: Unable to delete accepted appointment.');</script>";
			}
...
<html>
<head>
<title>Elegance Salon || Accepted Appointment</title>
	...
	<!-- Classie -->
		...
		<script>
            ...
            if (confirm("Are you sure you want to delete this accepted-appointment?")) {
                window.location.href = "accepted-appointment.php?action=delete&id=" + id + "&csrf=" + csrfToken;
            }
        }
